...
class JarvisHUDOverlay:
    ...
    def _on_escape_pressed(self, event=None):
        """Handles user hitting ESC key: cancels active task and resets state."""
        if self.on_cancel_task:
            self.on_cancel_task()
        self.set_mode("idle")
        self.update_thought("⛔ Task cancelled by user (Escape).")
        self.update_badges(["Cancelled", "Idle"])
    ...
    def _build_ui(self):
        ...
        # 5. Quick Action Ribbon & Proactive Suggestion Box
        self.quick_ribbon = ctk.CTkFrame(self.main_card, fg_color="transparent")
        self.quick_ribbon.pack(fill="x", padx=16, pady=(4, 6))

        btn_inspect = ctk.CTkButton(
            self.quick_ribbon, text="📊 Analyze Screen", font=(THEME["font_family"], 11),
        ...
        btn_undo = ctk.CTkButton(
            self.quick_ribbon, text="⏪ Undo Action", font=(THEME["font_family"], 11),
        ...
        # Proactive Ambient Triggers Container (Downloads & Clipboard suggestions)
        self.proactive_container = ctk.CTkFrame(self.main_card, fg_color="transparent")
        self.proactive_container.pack(fill="x", padx=16, pady=(0, 10))
    ...
    # ------------------------------------------------------------------
    # Public API (called from the agent core or background threads)
    # ------------------------------------------------------------------
    ...
    def show_proactive_suggestion(self, source: str, title: str, actions: List[Dict[str, str]]):
        """Thread-safe display of proactive suggestions from ambient watchers."""
        if not self.root:
            return
        try:
    ...
    def _show_proactive_suggestion(self, source: str, title: str, actions: List[Dict[str, str]]):
        ...
        card = ctk.CTkFrame(
            ...
            fg_color="#101e38",
            border_color=THEME["accent_amber"] if source == "clipboard" else THEME["accent_green"],
            border_width=1,
            corner_radius=8
        )
        card.pack(fill="x")

        # Top line: Title & Dismiss button
        top = ctk.CTkFrame(card, fg_color="transparent")
        top.pack(fill="x", padx=10, pady=(6, 4))

        lbl = ctk.CTkLabel(
            top, text=title, font=(THEME["font_family"], 11, "bold"),
            text_color=THEME["text_primary"], anchor="w"
        )
        lbl.pack(side="left")

        dismiss_btn = ctk.CTkButton(
            top, text="✕", width=20, height=20, fg_color="transparent",
            hover_color="#3a1c28", text_color=THEME["text_secondary"],
            command=self._clear_proactive_suggestion
        )
        ...
        for act in actions:
            btn = ctk.CTkButton(
                btn_row, text=act["label"], font=(THEME["font_family"], 10, "bold"),
                fg_color=THEME["accent_cyan"], text_color="#000000",
                hover_color=THEME["accent_glow"], height=24,
    ...
    def update_badges(self, badges: List[str]):
        """Thread-safe badge update."""
        if not self.root:
            return
        try:
            self.root.after(0, lambda: self._update_badges(badges))
        except Exception:
            pass
