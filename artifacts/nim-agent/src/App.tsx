...
import { ArrowRight, ArrowUpRight, Braces, Check, Chrome, CircleDot, Code2, Compass, Download, Eye, Fingerprint, Globe2, Layers3, LockKeyhole, Menu, MousePointer2, Network, PanelTop, Pause, Play, ScanSearch, Search, ShieldAlert, ShieldCheck, Terminal, X, Zap, type LucideIcon } from 'lucide-react';
    ...
    <header className="fixed left-0 right-0 top-0 z-40 border-b border-[#10203a]/10 bg-[#f4f2ec]/90 backdrop-blur-xl">
      <div className="mx-auto flex h-[72px] max-w-[1240px] items-center justify-between px-5 lg:px-8">
        ...
        <div className="hidden items-center gap-3 md:flex">
          ...
          <button onClick={() => onDownload('Chrome')} className="focus-ring group flex items-center gap-2 rounded-full bg-[#10203a] px-4 py-2.5 text-sm font-semibold text-[#f4f2ec] transition hover:bg-[#263b59]" data-testid="button-nav-download">
            <Chrome size={15} /> Add to Chrome <ArrowUpRight size={14} className="transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" />
          </button>
        </div>
      ...
      {open && <div className="border-t border-[#10203a]/10 bg-[#f4f2ec] px-5 py-5 md:hidden">
        <div className="grid gap-2">
          ...
          <button onClick={() => { setOpen(false); onDownload('Chrome'); }} className="mt-3 flex items-center justify-center gap-2 rounded-full bg-[#10203a] px-4 py-3 text-sm font-semibold text-[#f4f2ec]" data-testid="button-mobile-download"><Download size={15} /> Add to Chrome</button>
        </div>
      </div>}
    </header>
  );
        ...
        <div className="mb-14 grid items-end gap-12 lg:grid-cols-[.9fr_1.1fr] lg:gap-20">
          <div className="nim-rise" style={{ animationDelay: '.05s' }}>
            ...
            <div className="mt-9 flex flex-wrap items-center gap-3">
              <button onClick={() => onDownload('Chrome')} className="focus-ring group flex items-center gap-3 rounded-full bg-[#df6b48] px-5 py-3.5 text-sm font-semibold text-[#f9f3ec] shadow-[0_10px_25px_rgba(223,107,72,.22)] transition hover:-translate-y-0.5 hover:bg-[#cc5d3e]" data-testid="button-hero-chrome"><Chrome size={17} /> Add to Chrome <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" /></button>
              <button onClick={() => onDownload('Edge')} className="focus-ring flex items-center gap-2 rounded-full border border-[#10203a]/20 px-5 py-3.5 text-sm font-semibold text-[#10203a] transition hover:border-[#10203a]/50 hover:bg-[#10203a]/5" data-testid="button-hero-edge"><Download size={16} /> Get for Edge</button>
            </div>
  ...
  return (
    <section className="bg-[#e4e9e3] px-5 py-24 lg:px-8 lg:py-32">
      <div className="mx-auto grid max-w-[1240px] items-center gap-16 lg:grid-cols-[.85fr_1.15fr]">
        <div><div className="mono mb-5 text-[10px] uppercase tracking-[.2em] text-[#62877e]">Under the hood / 04</div><h2 className="text-[clamp(2.8rem,5vw,5.2rem)] font-semibold leading-[.93] tracking-[-.065em] text-[#10203a]">Thin by design.<br /><span className="serif font-normal italic text-[#62877e]">Sharp by default.</span></h2><p className="mt-7 max-w-[460px] text-[16px] leading-7 text-[#536071]">NIM lives where the work lives: inside your browser. No developer backend, no proxy, no mysterious copy of your session.</p><a href="#safety" className="focus-ring mt-8 inline-flex items-center gap-2 text-sm font-semibold text-[#10203a] underline decoration-[#df6b48] decoration-2 underline-offset-4 transition hover:text-[#df6b48]" data-testid="link-architecture-safety">See the safety model <ArrowUpRight size={15} /></a></div>
...
function Safety() {
  ...
  return <section id="safety" className="border-y border-[#bfc9c1] bg-[#e7ece5] px-5 py-24 lg:px-8 lg:py-32"><div className="mx-auto grid max-w-[1240px] gap-14 lg:grid-cols-[.75fr_1.25fr] lg:gap-24"><div><div className="mono mb-5 text-[10px] uppercase tracking-[.2em] text-[#62877e]">The trust layer / 06</div><h2 className="text-[clamp(2.8rem,5vw,5.3rem)] font-semibold leading-[.93] tracking-[-.065em] text-[#10203a]">Power with<br /><span className="serif font-normal italic text-[#62877e]">a handbrake.</span></h2><p className="mt-7 max-w-[390px] text-[16px] leading-7 text-[#536071]">Automation should extend your agency, never quietly replace it. NIM makes the boundary part of the interface.</p></div><div className="grid gap-x-8 gap-y-10 sm:grid-cols-2">{items.map(([title, body, Icon], i) => <div key={title} className="reveal-ready border-t border-[#b7c5bd] pt-5" style={{ transitionDelay: `${i * 90}ms` }}><div className="mb-5 flex items-center justify-between"><Icon size={21} className="text-[#df6b48]" /><span className="mono text-[9px] text-[#899790]">0{i + 1}</span></div><h3 className="text-[16px] font-semibold text-[#10203a]">{title}</h3><p className="mt-2 text-[13px] leading-6 text-[#687570]">{body}</p></div>)}</div></div></section>;
}

function DownloadSection({ onDownload }: { onDownload: (browser: string) => void }) {
  return <section id="download" className="relative overflow-hidden bg-[#df6b48] px-5 py-24 text-[#10203a] lg:px-8 lg:py-32"><div className="absolute right-[-8%] top-[-30%] h-[480px] w-[480px] rounded-full border-[1px] border-[#f2b49d]/45" /><div className="absolute right-[2%] top-[-15%] h-[340px] w-[340px] rounded-full border-[1px] border-[#f2b49d]/35" /><div className="relative mx-auto max-w-[1240px]"><div className="max-w-[780px]"><div className="mono mb-6 text-[10px] uppercase tracking-[.2em] text-[#784434]">Make the browser yours / 07</div><h2 className="text-[clamp(3.5rem,8vw,8rem)] font-semibold leading-[.85] tracking-[-.08em]">Open a better<br /><span className="serif font-normal italic">way through.</span></h2><p className="mt-8 max-w-[450px] text-[16px] leading-7 text-[#784434]">NIM is free to start, local by default, and ready when you are. Install the extension and give it one good task.</p><div className="mt-9 flex flex-wrap gap-3"><button onClick={() => onDownload('Chrome')} className="focus-ring flex items-center gap-3 rounded-full bg-[#10203a] px-5 py-3.5 text-sm font-semibold text-[#f4f2ec] transition hover:bg-[#263b59]" data-testid="button-download-chrome"><Chrome size={17} /> Add to Chrome <ArrowUpRight size={15} /></button><button onClick={() => onDownload('Edge')} className="focus-ring flex items-center gap-3 rounded-full border border-[#10203a]/35 px-5 py-3.5 text-sm font-semibold transition hover:bg-[#10203a]/10" data-testid="button-download-edge"><Download size={16} /> Get for Edge</button></div></div><div className="mt-20 flex flex-wrap gap-x-8 gap-y-3 border-t border-[#a9503c]/35 pt-5 text-[10px] uppercase tracking-[.13em] text-[#784434]"><span>Chrome + Edge</span><span>Local-first</span><span>Bring your provider</span><span>Human approval built in</span></div></div></section>;
}

function Footer() {
  return <footer className="bg-[#10203a] px-5 py-10 text-[#f4f2ec] lg:px-8"><div className="mx-auto flex max-w-[1240px] flex-col justify-between gap-8 md:flex-row md:items-end"><div><Logo /><p className="mt-5 max-w-[250px] text-[12px] leading-5 text-[#8699a0]">A browser agent for people who want more from the open web.</p></div><div className="flex flex-wrap gap-5 text-[11px] text-[#9cafad]"><a href="#capability" className="transition hover:text-[#f4f2ec]" data-testid="link-footer-capability">Capability</a><a href="#safety" className="transition hover:text-[#f4f2ec]" data-testid="link-footer-safety">Safety</a><a href="#download" className="transition hover:text-[#f4f2ec]" data-testid="link-footer-download">Download</a><a href="mailto:hello@nim.agent" className="transition hover:text-[#f4f2ec]" data-testid="link-footer-contact">Contact</a></div><div className="mono text-[9px] uppercase tracking-[.13em] text-[#627782]">© 2025 NIM Agent</div></div></footer>;
}
